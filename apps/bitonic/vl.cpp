...
void swap_worker(const int desired_core) {
  ...
  // open endpoints
  if ((errorcode = open_twin_vl_as_consumer(to_swap_fd, &to_swap_cons, 1))) {
    printf("\033[91mFAILED:\033[0m %s(), to_swap_cons\n", __func__);
    return;
  }
  if ((errorcode = open_twin_vl_as_producer(to_swap_fd, &to_swap_prod, 1))) {
    printf("\033[91mFAILED:\033[0m %s(), to_swap_prod\n", __func__);
    return;
  }
  if ((errorcode =
       open_twin_vl_as_producer(to_connect_fd, &to_conn_prod, 1))) {
    printf("\033[91mFAILED:\033[0m %s(), to_conn_prod\n", __func__);
    return;
  }

  while (!done) {
    twin_vl_pop_non(&to_swap_cons, &arr_base, &isvalid);
    if (isvalid) {
      twin_vl_pop_weak(&to_swap_cons, &arr_len);
      twin_vl_pop_weak(&to_swap_cons, &arr_beg);
      twin_vl_pop_weak(&to_swap_cons, &arr_end);
      ...
      if (2 < len_tmp) {
        ...
        twin_vl_push_weak(&to_swap_prod, arr_base);
        twin_vl_push_weak(&to_swap_prod, arr_len);
        twin_vl_push_weak(&to_swap_prod, arr_beg);
        twin_vl_push_weak(&to_swap_prod, arr_end);
        twin_vl_flush(&to_swap_prod);
        arr_beg += len_tmp;
        arr_end += len_tmp;
        twin_vl_push_weak(&to_swap_prod, arr_base);
        twin_vl_push_weak(&to_swap_prod, arr_len);
        ...
        twin_vl_push_weak(&to_conn_prod, arr_base);
        twin_vl_push_weak(&to_conn_prod, arr_len);
        twin_vl_push_weak(&to_conn_prod, arr_beg);
        twin_vl_push_weak(&to_conn_prod, arr_end);
        twin_vl_flush(&to_conn_prod);
      }
    }
    done = lock.done;
  }
...
void rswap_worker(const int desired_core) {
  ...
  // open endpoints
  if ((errorcode =
       open_twin_vl_as_consumer(to_rswap_fd, &to_rswap_cons, 1))) {
  ...
  while (!done) {
    twin_vl_pop_non(&to_rswap_cons, &arr_base, &isvalid);
    if (isvalid) {
      twin_vl_pop_weak(&to_rswap_cons, &arr_len);
      twin_vl_pop_weak(&to_rswap_cons, &arr_beg);
      twin_vl_pop_weak(&to_rswap_cons, &arr_end);
      int *arr_tmp = (int *)arr_base;
      arr_tmp = &arr_tmp[arr_beg];
      uint64_t len_tmp = arr_end - arr_beg;
...
void connector(const int desired_core, const uint64_t len) {
  ...
  // open endpoints
  if ((errorcode =
       open_twin_vl_as_consumer(to_connect_fd, &to_conn_cons, 1))) {
    printf("\033[91mFAILED:\033[0m %s(), to_conn_cons\n", __func__);
    return;
  }
  if ((errorcode =
       open_twin_vl_as_producer(to_rswap_fd, &to_rswap_prod, 1))) {
    printf("\033[91mFAILED:\033[0m %s(), to_rswap_prod\n", __func__);
    return;
  }

  while (true) {
    twin_vl_pop_non(&to_conn_cons, &arr_base, &isvalid);
    if (isvalid) {
      twin_vl_pop_weak(&to_conn_cons, &arr_len);
      twin_vl_pop_weak(&to_conn_cons, &arr_beg);
      twin_vl_pop_weak(&to_conn_cons, &arr_end);
      ...
      if (connected) {
        ...
        if (pcount[idx_1st] == pcount[idx_2nd]) {
          twin_vl_push_weak(&to_rswap_prod, arr_base);
          twin_vl_push_weak(&to_rswap_prod, arr_len);
          twin_vl_push_weak(&to_rswap_prod, idx_1st);
          twin_vl_push_weak(&to_rswap_prod, idx_2nd + len_to_connect);
          twin_vl_flush(&to_rswap_prod);
        } // if (paird)
      } // if (connected)
    } // if (isvalid)
  } // while (true)
...
void sort(int *arr, const uint64_t len) {
  ...
  if ((errorcode = open_twin_vl_as_producer(to_swap_fd, &endpt, 1))) {
    printf("\033[91mFAILED:\033[0m open_twin_vl_as_producer(to_swap_fd) "
           "return %d\n", errorcode);
    return;
  }
  ...
  for (uint64_t i = 0; len > i; i += 2) {
    twin_vl_push_strong(&endpt, (uint64_t)arr);
    twin_vl_push_strong(&endpt, len);
    twin_vl_push_strong(&endpt, i);
    twin_vl_push_strong(&endpt, i + 2);
    twin_vl_flush(&endpt);
  }
